...
-export([stop/1, start_link/2, init/1, terminate/2, code_change/3, handle_info/2,
         get_tick/0]).
...
handle_cast(tick, State) ->
    ...
    NewState =
        case adamantum_chunk_manager:update_loaded_player_chunks(DB_player#db_player.coords,
                                                                 State#state.loaded_chunks)
        of
    ...
    {noreply, NewState};
%c534f6a0-7882-3f79-8143-0107ae25aba5
    ...
    NewState =
        case Req of
            ...
            {login_start, [Username]} ->
                UUID_full = adamantum_uuid:get_uuid(Username),
                io:format("~p~n", [UUID_full]),
                DB_player =
                    case adamantum_player_manager:read_from_db(UUID_full) of
                        [] ->
                            io:format("~p~n", ["new player"]),
                            Player =
                                #db_player{uuid = UUID_full,
                                           username = Username,
                                           gamemode = 1,
                                           eid = 330,
                                           coords = {0, 100, 0, 0, 0},
                                           current_slot = 0,
                                           dimension = 0},
                            adamantum_player_manager:write_to_db(UUID_full, Player),
                            Player;
                        DB ->
                            Player =
                                #db_player{uuid = UUID_full,
                ...
                encode_message([UUID_full, binary_to_list(Username)], login_success),
                encode_message([DB_player#db_player.eid,
                                DB_player#db_player.gamemode,
                                DB_player#db_player.dimension,
                                2,
                                20,
                                "flat",
                                false],
                ...
                adamantum_player_manager:connect_to_PM(UUID_full, self()),
                State#state{state_of_play = 3, db_key = UUID_full};
            ...
            {player_position, [X, Y, Z, _On_ground]} ->
                ...
                adamantum_player_manager:write_to_db(State#state.db_key,
                                                     Db_player#db_player{coords =
                                                                             {X,
                                                                              Y,
                                                                              Z,
                                                                              Yaw,
            ...
            {player_position_and_look, [X, Y, Z, Yaw, Pitch, _On_ground]} ->
                ...
                State;
            %player actions
            {player_digging, [Status, {X, Y, Z}, _Face]} ->
                case Status of
                    0 ->
                        ok;
                    1 ->
                        ok;
                    2 ->
                        adamantum_chunk_manager:remove_block({X, Y, Z});
                    _ ->
                        ok
                end,
                State;
            ...
            {tab_complete, [_Text, _Has_position, {_X, _Y, _Z}]} ->
                State;
            {client_settings,
             [_Locale, _View_distance, _Chat_mode, _Chat_colors, _Displayed_skin_parts]} ->
                State;
...
seperate_chunk_data_into_parts([], Chunks_to_add, DB_player, State) ->
    ...
    case DB_player#db_player.dimension of
        0 ->
            send_message({map_chunk_bulk,
                          <<38:8, 1:8, Number_of_chunks/binary, Chunks_to_send/binary>>},
                         State);
        _ ->
            send_message({map_chunk_bulk,
                          <<38:8, 0:8, Number_of_chunks/binary, Chunks_to_send/binary>>},
                         State)
    end,
